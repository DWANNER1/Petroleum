...
echo Deploying on Lightsail without DB reseed...
ssh -i "%KEY%" %HOST% "bash -lc 'set -e; echo [1/6] unzip frontend; unzip_status=0; unzip -o %REMOTE_HOME%/web-mui-dist.zip -d %REMOTE_REPO%/apps/web-mui/dist || unzip_status=$?; if [ $unzip_status -gt 1 ]; then exit $unzip_status; fi; echo [2/6] npm install; cd %REMOTE_REPO% && npm install; echo [3/6] install nginx config; sudo cp %REMOTE_REPO%/wl-portal.nginx.conf /etc/nginx/sites-available/wl-portal; sudo ln -sf /etc/nginx/sites-available/wl-portal /etc/nginx/sites-enabled/wl-portal; echo [4/6] restart api; fuser -k 4000/tcp || true; cd %REMOTE_REPO% && nohup env NODE_OPTIONS=--max-old-space-size=256 node apps/api/src/server.js > api.log 2>&1 & sleep 3; echo [5/6] reload nginx; sudo nginx -t; sudo systemctl reload nginx; echo [6/6] health check; curl http://127.0.0.1:4000/health'"
if errorlevel 1 goto :fail
